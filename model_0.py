...
def plot_places_on_map(places, user_location):
    if not places:
        print("No places to display on the map.")
        return
    ...
    category_icons = {
        ...
        "Chinese": "bowl-food",
        "Italian": "pizza-slice",
        "American": "drumstick-bite",
        "Middle Eastern": "bread-slice",
        "Other": "cutlery"
    }
    ...
    for place in places:
        ...
        if rating not in ["N/A", None]:
            try:
                ...
                marker_color = "green" if rating_float >= 4.0 else "orange" if rating_float >= 3.0 else "red"
        ...
        🥗 Food: {food_types}<br>
        ⭐ Rating: {rating} ({place["rating_count"]} reviews)<br>
        📏 Distance: {distance} km<br>
        <a href="https://www.google.com/maps/search/{name.replace(' ', '+')}+@{lat},{lng}" target="_blank">🗺 Navigate</a>
        """
        ...
        folium.Marker(
            location=(lat, lng),
            popup=popup_text,
            icon=folium.Icon(color=marker_color, icon=category_icons.get(category, "cutlery")),
        ).add_to(marker_cluster)

    map_filename = "food_places_map.html"
...
if __name__ == "__main__":
    ...
    if filtered_results:
        ...
        plot_places_on_map(filtered_results, user_location)
    else:
